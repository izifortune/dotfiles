...
function obj:updateMicMute(muted)
    ...
    muted = hs.audiodevice.defaultInputDevice():volume() == 5
	end
	if muted then
...
-- local wf_teams = wf.new(false):setAppFilter('Microsoft Teams',{allowTitles='| Microsoft Teams'}) -- any Safari windows with "reddit" anywhere in the title
-- wf_teams:subscribe(wf.hasWindow, function() print('window') end)

function isTeamsCallOn()
  return false
  -- local wins = hs.window.allWindows()
  -- local isTeamsCallOn = false
  -- for i, x in pairs(wins) do
  --   if (string.find(x:title(),"| Microsoft Teams")) then
  --     isTeamsCallOn = true
  --   end
  -- end
  -- return isTeamsCallOn
end
...
function obj:toggleMicMute()
  ...
  if (teams and isTeamsCallOn() and false)  then
    hs.eventtap.keyStroke({"cmd", "shift"}, "m", 0, teams)
    print('Mute teams only')
  else 
    if mic:volume() == 5 then
      -- mic:setInputMuted(false)
      mic:setInputVolume(80)
      if zoom then
